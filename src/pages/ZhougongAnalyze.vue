        ...
        <div class="content-wrap">
            ...
            <div class="bottom">
                ...
                <div class="bottom-list">
                    <div class="tag-item" v-for="(item,index) in objList" :key="index" @click="result(item.id)" :view-id="newRender">
                        {{item.title}}
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods:{
        getData: function(id){
            this.$http.post('/suan/apidata',id,'cesuan',null,this.success,this.failure);
        },
        ...
        // 获取底部随机推荐数据
        randomData: function () {
            let list = this.$route.params.objList;
            for(let i=0; i<10; i++){
                let j = parseInt(Math.random()*50);
                // console.log(list[j])
                this.objList.push(list[j])
            }
        },
        //跳转空路由再跳转回来模拟刷新页面效果
        result :function (id) {
